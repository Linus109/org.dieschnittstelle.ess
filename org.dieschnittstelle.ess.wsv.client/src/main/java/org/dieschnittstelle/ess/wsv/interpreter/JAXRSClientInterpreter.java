...
public class JAXRSClientInterpreter implements InvocationHandler {
    ...
    public JAXRSClientInterpreter(Class serviceInterface,String baseurl) {
        ...
        if (serviceInterface.isAnnotationPresent(Path.class)) {
            ...
            throw new RuntimeException("Cannot construct invocation handler, JAX-RS root resources require a top-level @Path annotation");
        }
    ...
    public Object invoke(Object proxy, Method meth, Object[] args)
        ...
        if ("toString".equals(meth.getName())) {
            return "JAX-RX client proxy for " + this.serviceInterface.getName();
        }
        ...
        if (args != null && args.length > 0) {
            if (meth.getParameterAnnotations()[0].length > 0 && meth.getParameterAnnotations()[0][0].annotationType() == PathParam.class) {
                ...
                show("pathParam: " + pathParam + "; pathValue: " + args[0]);
        ...
        if (meth.isAnnotationPresent(GET.class)) {
            ...
            throw new UnsupportedOperationException("Only GET, POST, PUT and DELETE are considered but not " + meth.getName());
        }
        ...
        if (requestBodyData != null) {
            ...
            ByteArrayOutputStream requestBodyJsonContainer = new ByteArrayOutputStream();

            // TODO: write the object to the stream using the jsonSerialiser
            jsonSerialiser.writeObject(requestBodyData, requestBodyJsonContainer);

            // TODO: create an ByteArrayEntity from the stream's content, assiging it to requestBodyDataAsJson
            requestBodyDataAsJson = new ByteArrayEntity(requestBodyJsonContainer.toByteArray());
        ...
        if (response.getStatusLine().getStatusCode() == HttpStatus.SC_OK) {
            ...
            InputStream returnValueData = response.getEntity().getContent();

            returnValue = jsonSerialiser.readObject(returnValueData, returnValueType);
