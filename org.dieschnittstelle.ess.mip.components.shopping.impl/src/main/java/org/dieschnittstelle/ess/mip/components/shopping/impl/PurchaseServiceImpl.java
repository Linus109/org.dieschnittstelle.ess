...
public class PurchaseServiceImpl implements PurchaseService {
    ...
    private void checkAndRemoveProductsFromStock() {
        ...
        for (ShoppingCartItem item : this.shoppingCart.getItems()) {
            ...
            if (product == null) {
                show("product with id: " + item.getErpProductId() + " an does not exist at all");
                return;
            }
            ...
            if (item.isCampaign()) {
                ...
                for (ProductBundle bundle : campaign.getBundles()) {
                    int removeCount = bundle.getUnits() * item.getUnits();
                    removeFromStockIfPossible(bundle.getProduct(), removeCount);
                }
            } else {
                ...
                IndividualisedProductItem individualisedProductItem = (IndividualisedProductItem) product;
                removeFromStockIfPossible(individualisedProductItem, item.getUnits());
            }
    ...
    public void purchaseCartAtTouchpointForCustomer(long shoppingCartId, long touchpointId, long customerId) throws ShoppingException {
        ...
        for (ShoppingCartItem item : this.shoppingCartService.getItems(shoppingCartId)) {
